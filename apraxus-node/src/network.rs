...
impl Network {
    ...
    pub fn connect_to_peer(
        ...
        let peer_blockchain =
            match serde_json::from_slice::<Blockchain>(
                &blockchain_data,
            ) {
                ...
                Err(error) => {
                    println!(
                        "❌ Failed to decode peer blockchain: {}",
                        error
                    );
        ...
        if !local_exists {
            println!(
                "🆕 No local blockchain found."
            );

            println!(
                "📥 Adopting peer blockchain..."
            );

            match peer_blockchain.save_to_file(
                &self.blockchain_file
            ) {
                Ok(()) => {
                    println!(
                        "💾 Peer blockchain saved locally."
                    );
                    ...
                    println!(
                        "✅ Initial blockchain synchronization complete."
                    );
                }

                Err(error) => {
                    println!(
                        "❌ Failed to save peer blockchain: {}",
                        error
                    );
                }
            }
        ...
        let mut local_blockchain =
            match Blockchain::load_from_file(
                ...
                Err(error) => {
                    println!(
                        "❌ Failed to load local blockchain: {}",
                        error
                    );
        ...
        match local_blockchain.save_to_file(
            &self.blockchain_file
        ) {
            Ok(()) => {
                println!(
                    "💾 Synchronized blockchain saved."
                );
            ...
            Err(error) => {
                println!(
                    "❌ Failed to save synchronized blockchain: {}",
                    error
                );
            }
        }
    ...
    fn handle_connection(
        ...
        match stream.read_to_end(
            ...
            Ok(bytes_read) if bytes_read > 0 => {
                ...
                {
                    println!(
                        "📨 Received from peer: GET_BLOCKCHAIN"
                    );
                    ...
                    )
                {
                    ...
                    let transaction =
                        match serde_json::from_str::<SignedTransaction>(
                            json,
                        ) {
                            Ok(transaction) => {
                                transaction
                    ...
                    match blockchain.save_to_file(
                        ...
                        Ok(()) => {
                            println!(
                                "💾 Transaction pool saved successfully."
                            );
                        }
                ...
                println!(
                    "⚠️ Unknown message received from peer."
                );
            }

            Ok(_) => {
                println!(
                    "⚠️ Peer closed the connection."
                );
            }
    ...
    fn send_blockchain(
        ...
        if !blockchain.is_chain_valid() {
            println!(
                "❌ Local blockchain is invalid. Not sending."
            );
